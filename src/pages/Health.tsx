    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="px-4 py-6 pb-24 animate-fade-in">
        <div className="space-y-6">
          ...
          {/* Selected Pet Info */}
          {selectedPetData && (
            <Card className="p-4 rounded-2xl pet-card-shadow">
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-pet-primary/20 to-pet-secondary/20 flex items-center justify-center text-xl">
                  {selectedPetData.avatar.startsWith('data:') || selectedPetData.avatar.startsWith('http') ? (
                    <img 
                      src={selectedPetData.avatar} 
                      alt={selectedPetData.name} 
                      className="w-full h-full object-cover rounded-xl"
                    />
                  ) : (
                    getSpeciesEmoji(selectedPetData.species)
                  )}
                </div>
                <div>
                  <h3 className="font-semibold text-pet-primary">{selectedPetData.name}</h3>
                  <p className="text-sm text-muted-foreground">{selectedPetData.breed} • {selectedPetData.age}</p>
                </div>
              </div>
            </Card>
          )}
